...
{enum BMI160_Status s = (status); if (s != BMI160_STATUS_OK) return s;}
...
{
	h->tx[0] = BMI160_read_command(BMI160_REG_DUMMY);
	BMI160_RETURN_IF_NOT_OK(BMI160_write_read(h, 2));
	BMI160_wait(h, 2);
	return BMI160_STATUS_OK;
}

static enum BMI160_Status BMI160_softreset(
	struct BMI160_Handle *h)
{
	h->tx[0] = BMI160_write_command(BMI160_REG_CMD);
	h->tx[1] = BMI160_CMD_COMMAND_SOFTRESET;
	BMI160_RETURN_IF_NOT_OK(BMI160_write_read(h, 2));
	BMI160_wait(h, 20);
	return BMI160_STATUS_OK;
}
...
static enum BMI160_Status BMI160_check_id(
	struct BMI160_Handle *h)
{
	h->tx[0] = BMI160_read_command(BMI160_REG_CHIP_ID);
	BMI160_RETURN_IF_NOT_OK(BMI160_write_read(h, 2));
	if (h->rx[1] != BMI160_REG_DEFAULT_CHIP_ID) {
...
enum BMI160_Status BMI160_Handle_init(struct BMI160_Handle *h)
{
	...
	BMI160_RETURN_IF_NOT_OK(BMI160_softreset(h));
	BMI160_RETURN_IF_NOT_OK(BMI160_activate_spi(h));
